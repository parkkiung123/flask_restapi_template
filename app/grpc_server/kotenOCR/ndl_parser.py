...
categories = [
    ...
    {'id': int(Category.LINE_NOTE),    'name': 'line_note',    'org_name': '注'},
    {'id': int(Category.LINE_NOTE_DUMMY),    'name': 'line_note_dummy',    'org_name': '注（ダミー）'},
    ...
    for i, child_poly in enumerate(res_polygons):
        ...
        for j, parent_poly in enumerate(res_polygons):
            ...
            for p in child_poly:
                ...
                if point_in_polygon(parent_poly, (int(x), int(y)),False)< -margin:
    ...
    for c in range(len(classes)):
        ...
        for j, line in enumerate(res_bbox[c]):
            ...
            for i, poly in enumerate(tb_polygons):
                ...
                if point_in_polygon(poly, (int(cx), int(cy)),False)>=0:
...
def get_relationship_rect(res_bbox, tb_polygons, classes, use_block_ad: bool = False, score_thr: float = 0.3):
    #import cv2
    tb_cls_id = classes.index('text_block')
    tb_info = [[] for i in range(len(tb_polygons))]
    independ_lines = []
    ...
    for c_index, child_poly in enumerate(tb_polygons):
        ...
        for p_index, parent_poly in enumerate(tb_polygons):
            ...
            for p in child_poly:
                ...
                if point_in_polygon(poly, (int(cx), int(cy)),True)<-margin:
...
def convert_to_xml_string3_custom(img_w, img_h, classes, result,
                           score_thr: float = 0.3,
                           min_bbox_size: int = 5,
                           use_block_ad: bool = False):
    #import cv2

    s = f'<PAGE WIDTH = "{img_w}" HEIGHT = "{img_h}">\n'
